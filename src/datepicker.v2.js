class DatePicker {
  ...
  static initializeDatePicker() {
    if (!DatePicker.datepicker) {
      ...
      DatePicker.datepicker.innerHTML = `
        <div yawdc-datepicker-header>
          <div yawdc-normal-header>
            <select yawdc-month-select-hidden hidden></select>
            <select yawdc-year-select-hidden hidden></select>
            <div yawdc-datepicker-year-month>
              <button yawdc-action-button yawdc-month-select></button>
              <button yawdc-action-button yawdc-year-select></button>
            </div>
          </div>
          <div yawdc-year-header>Select a year</div>
          <div yawdc-month-header>Select a month</div>
        </div>

        <div yawdc-datepicker-calendar>
          <div yawdc-datepicker-weekdays>
            <span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span>
          </div>
          <div yawdc-datepicker-days></div>
        </div>
        <div yawdc-datepicker-years style="overflow-y: auto;"></div>
        <div yawdc-datepicker-months class="month-grid"></div>
      `;
  ...
  setupListeners() {
    ...
    this.inputElement.addEventListener('click', event => {
      ...
      DatePicker.activeInput = this.inputElement;
    ...
    document.addEventListener('click', event => {
      if (!DatePicker.datepicker.contains(event.target) && event.target !== DatePicker.activeInput) {
  ...
  shouldTriggerMobileLayout() {
    ...
    if (viewPortWidth <= this.mobileLayout) {
      DatePicker.datepicker.style.top = '50%';
